...
public class UserJpaRepositoryTest {
    ...
    void findByEmailAndStatus_로_유저_데이터를_찾아올_수_있다() {
        Optional<User> result = userJpaRepository.findByEmailAndStatus("kok202@naver.com", UserStatus.ACTIVE);
        // then
        assertThat(result.isPresent()).isTrue();
    }
    ...
    void findByEmailAndStatus_는_데이터가_없으면_Optional_empty를_내려준다() {
        Optional<User> result = userJpaRepository.findByEmailAndStatus("deveun@naver.com", UserStatus.PENDING);
        // then
        assertThat(result.isEmpty()).isTrue();
    }
